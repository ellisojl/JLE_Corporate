 ...
 */
class JLE_Corporate_Block_Adminhtml_Attribute_Edit_Tab_Main
    ...
    implements Mage_Adminhtml_Block_Widget_Tab_Interface
{
     ...
     *
     * @return JLE_Corporate_Block_Adminhtml_Attribute_Edit_Tab_Main
     */
    protected function _prepareForm()
    {
        ...
        /* @var $helper jle_corporate_Helper_Data */
        ...
        $element->setLabel(Mage::helper('jle_corporate')->__('Input Type'));
        ...
        $fieldset->addField('multiline_count', 'text', array(
            ...
            'label'     => Mage::helper('jle_corporate')->__('Lines Count'),
            'title'     => Mage::helper('jle_corporate')->__('Lines Count'),
            'required'  => true,
            'class'     => 'validate-digits-range digits-range-2-20',
            'note'      => Mage::helper('jle_corporate')->__('Valid range 2-20')
        ), 'frontend_input');

        $fieldset->addField('input_validation', 'select', array(
            'name'      => 'input_validation',
            'label'     => Mage::helper('jle_corporate')->__('Input Validation'),
            'title'     => Mage::helper('jle_corporate')->__('Input Validation'),
            'values'    => array('' => Mage::helper('jle_corporate')->__('None'))
        ), 'default_value_textarea');

        $fieldset->addField('min_text_length', 'text', array(
            'name'      => 'min_text_length',
            'label'     => Mage::helper('jle_corporate')->__('Minimum Text Length'),
            'title'     => Mage::helper('jle_corporate')->__('Minimum Text Length'),
            'class'     => 'validate-digits',
        ), 'input_validation');

        $fieldset->addField('max_text_length', 'text', array(
            'name'      => 'max_text_length',
            'label'     => Mage::helper('jle_corporate')->__('Maximum Text Length'),
            'title'     => Mage::helper('jle_corporate')->__('Maximum Text Length'),
            'class'     => 'validate-digits',
        ), 'min_text_length');

        $fieldset->addField('max_file_size', 'text', array(
            'name'      => 'max_file_size',
            'label'     => Mage::helper('jle_corporate')->__('Maximum File Size (bytes)'),
            'title'     => Mage::helper('jle_corporate')->__('Maximum File Size (bytes)'),
            'class'     => 'validate-digits',
        ), 'max_text_length');

        $fieldset->addField('file_extensions', 'text', array(
            'name'      => 'file_extensions',
            'label'     => Mage::helper('jle_corporate')->__('File Extensions'),
            'title'     => Mage::helper('jle_corporate')->__('File Extensions'),
            'note'      => Mage::helper('jle_corporate')->__('Comma separated'),
        ), 'max_file_size');

        $fieldset->addField('max_image_width', 'text', array(
            'name'      => 'max_image_width',
            'label'     => Mage::helper('jle_corporate')->__('Maximum Image Width (px)'),
            'title'     => Mage::helper('jle_corporate')->__('Maximum Image Width (px)'),
            'class'     => 'validate-digits',
        ), 'file_extensions');

        $fieldset->addField('max_image_heght', 'text', array(
            'name'      => 'max_image_heght',
            'label'     => Mage::helper('jle_corporate')->__('Maximum Image Height (px)'),
            'title'     => Mage::helper('jle_corporate')->__('Maximum Image Height (px)'),
            'class'     => 'validate-digits',
        ), 'max_image_width');

        $fieldset->addField('input_filter', 'select', array(
            'name'      => 'input_filter',
            'label'     => Mage::helper('jle_corporate')->__('Input/Output Filter'),
            'title'     => Mage::helper('jle_corporate')->__('Input/Output Filter'),
            'values'    => array('' => Mage::helper('jle_corporate')->__('None')),
        ));

        $fieldset->addField('date_range_min', 'date', array(
            'name'      => 'date_range_min',
            'label'     => Mage::helper('jle_corporate')->__('Minimal value'),
            'title'     => Mage::helper('jle_corporate')->__('Minimal value'),
        ...
        $fieldset->addField('date_range_max', 'date', array(
            'name'      => 'date_range_max',
            'label'     => Mage::helper('jle_corporate')->__('Maximum value'),
            'title'     => Mage::helper('jle_corporate')->__('Maximum value'),
            'image'     => $this->getSkinUrl('images/grid-cal.gif'),
            'format'    => $helper->getDateFormat()
        ), 'date_range_min');
        ...
        $fieldset = $form->addFieldset('front_fieldset', array(
            'legend'    => Mage::helper('jle_corporate')->__('Frontend Properties')
        ));

        $fieldset->addField('is_visible', 'select', array(
            'name'      => 'is_visible',
            'label'     => Mage::helper('jle_corporate')->__('Show on Frontend'),
            'title'     => Mage::helper('jle_corporate')->__('Show on Frontend'),
            'values'    => $yesnoSource,
        ));

        $fieldset->addField('sort_order', 'text', array(
            'name'      => 'sort_order',
            'label'     => Mage::helper('jle_corporate')->__('Sort Order'),
            'title'     => Mage::helper('jle_corporate')->__('Sort Order'),
            'required'  => true,
            'class'     => 'validate-digits'
        ));
/*
        $fieldset->addField('used_in_forms', 'multiselect', array(
            'name'         => 'used_in_forms',
            'label'        => Mage::helper('jle_corporate')->__('Forms to Use In'),
            'title'        => Mage::helper('jle_corporate')->__('Forms to Use In'),
...
//            }
        ...
        Mage::dispatchEvent('jle_corporate_attribute_edit_tab_general_prepare_form', array(
            'form'      => $form,
            'attribute' => $attribute
        ));
    ...
    {
        return Mage::helper('jle_corporate')->__('Properties');
    }
